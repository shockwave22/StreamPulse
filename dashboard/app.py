...
class StreamPulseDashboard:
    ...
    def load_netflix_titles(self):
        """Load Netflix titles from database"""
    ...
    def get_aggregated_data(self, title: str, days: int = 7):
        """Get aggregated data for a title"""
        session = db_manager.get_session()
        try:
            # Get title ID
    ...
    def get_raw_content(self, title: str, platform: str = 'both', limit: int = 50):
        """Get raw tweets or comments for a title"""
        session = db_manager.get_session()
        try:
            netflix_title = session.query(NetflixTitle).filter_by(title=title).first()
            if not netflix_title:
    ...
    def create_sentiment_trend_chart(self, df):
        ...
        # Filter for sentiment platforms
    ...
    def create_survey_comparison_chart(self, df):
        """Create survey vs social sentiment comparison chart"""
        if df.empty:
            return go.Figure()
        
        # Get survey and social data
        ...
        # Normalize satisfaction score to -1 to 1 scale
        normalized_satisfaction = (avg_satisfaction - 3) / 2  # Convert 1-5 scale to -1 to 1
        
        comparison_data = pd.DataFrame({
            'Metric': ['Survey Satisfaction', 'Social Sentiment', 'Recommendation Rate'],
            'Score': [normalized_satisfaction, avg_social_sentiment, recommendation_rate * 2 - 1]  # Scale recommendation to -1 to 1
        })
    ...
    def create_wordcloud(self, title: str):
        """Create word cloud from recent content"""
    ...
    def run_dashboard(self):
        ...
        with tab5:
            ...
            # Platform selection for raw data
